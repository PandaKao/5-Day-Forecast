...
class WeatherService {
  ...
  private buildForecastArray(currentWeather: Weather, weatherData: any[]) {
    ...
    const forecastArray = Array.from(dailyForecastMap.values()).sort((a, b) =>
      new Date(a.dt_txt).getTime() - new Date(b.dt_txt).getTime()
    );

    const weatherForecastArray = forecastArray.map((forecast: any) =>
      new Weather(forecast, currentWeather.city)
    );
